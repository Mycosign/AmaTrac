...
class Courier(models.Model):
    ...
    def get_avatar_url(self):
        """Returns a properly formatted HTTPS URL for the avatar image"""
        if self.avatar and hasattr(self.avatar, 'url'):
            url = self.avatar.url
            return url.replace('http://', 'https://') if url.startswith('http://') else url
        return "https://res.cloudinary.com/dfkuixeji/image/upload/v1729838238/new1_kmqb70.png"

    def get_tracking_url(self):
        """Returns the tracking URL for this courier"""
        return f"/core/core/?tracking_id={self.tracking_id}"
